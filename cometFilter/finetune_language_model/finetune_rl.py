...
import sys
import os
sys.path.append('/home/becky/Documents/CommonSense_RL/transformer')
sys.path.append('/home/becky/Documents/CommonSense_RL/transformer/pytorch_transformers')
        ...
        if True:
            ...
            with open(file_path, 'r') as f:
                for row in f:
                    ...
                    orig_sent = row.split('*')
                    label = int(orig_sent[0].strip())  # 0 right, 1 wrong
                    orig_sent_sen1 = orig_sent[1]
                    orig_sent_sen2 = orig_sent[2]
                    orig_sent_lst = []
                    mask_lst = []
                    if '[Char_1]' in orig_sent_sen2 and '[Char_2]' not in orig_sent_sen2:
                        orig_sent_lst.append('* [Char_1] *' + ''.join(orig_sent[1:]))
                        mask_lst.append(orig_sent_sen2)
                    elif '[Char_2]' in orig_sent_sen2 and '[Char_1]' not in orig_sent_sen2:
                        orig_sent_lst.append('* [Char_2] *' + ''.join(orig_sent[1:]))
                        mask_lst.append(orig_sent_sen2)
                    elif '[Char_2]' in orig_sent_sen2 and '[Char_1]' in orig_sent_sen2:
                        orig_sent_lst.append('* [Char_2] *' + ''.join(orig_sent[1:]))
                        mask_lst.append(orig_sent_sen2)
                        orig_sent_lst.append('* [Char_1] *' + ''.join(orig_sent[1:]))
                        mask_lst.append(orig_sent_sen2)
                        orig_sent_lst.append('* [Char_1] [Char_2] *' + ''.join(orig_sent[1:]))
                        mask_lst.append(orig_sent_sen2)
                    else:
                        orig_sent_lst.append('**' + ''.join(orig_sent[1:]))
                        mask_lst.append(orig_sent_sen2)

                    for i, orig_sent in enumerate(orig_sent_lst):
                        orig_sent = orig_sent.lstrip()
                        orig_sent = orig_sent.rstrip()
                        orig_sent = orig_sent.replace("\n", "")
                        orig_sent_sen2 = mask_lst[i]

                        print('orig_sent',  orig_sent)
                        print('orig_sent_sen2', orig_sent_sen2)
                        ...
                        token_sent_sen1 = [-1 for i in range(len(token_sent)-len(token_sent_sen2))]


                        token_sent = [token_sent, token_sent_sen1+ token_sent_sen2]
                        if len(token_sent[0]) != len(token_sent[1]):
                            print('0----token_sent', token_sent[0])
                            print(tokenizer.convert_tokens_to_string(tokenizer.convert_ids_to_tokens(token_sent[0])))
                            print('1----token_sent', token_sent[1])
                            print(tokenizer.convert_tokens_to_string(tokenizer.convert_ids_to_tokens([i for i in token_sent[1] if i != -1])))
                        # print('token', token_sent)
...
def train(args, train_dataset, model, tokenizer, pho):
    ...
    for _ in train_iterator:
            ...
            for step, batch in enumerate(epoch_iterator): #batch is tensor with inputs tokens
                ...
                # print(a.shape)
                # print(a)
                # print('loss', loss, a(outputs[1].reshape(outputs[1].shape[1], outputs[1].shape[2])[:-1], inputs.reshape(inputs.shape[1])[1:]))
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                        ...
                        output_dir = os.path.join(args.output_dir) + '/' + str(global_step // args.save_steps)
    ...
    # context_file = open("/home/becky/Documents/CommonsenseStoryGen/cometFilter/RL_data/cached_dict.p", 'wb+')
...
def run_finetune_gpt2(gpu_use, pho, round, device=None):
    ...
    parser.add_argument("--train_data_file", default='/home/becky/Documents/CommonsenseStoryGen/cometFilter/finetune_language_model/data/continuation_old.txt', type=str,
                        required=False,
                        help="The input training data file (a text file).")
    parser.add_argument("--output_dir", default='finetuned_model/roc_rl', type=str, required=False,
    ...
    ##################################
    if round == 0:
        args.model_name_or_path = '/home/becky/Documents/CommonsenseStoryGen/cometFilter/finetune_language_model/finetuned_model/roc_char/21'
    else:
        args.model_name_or_path = '/home/becky/Documents/CommonsenseStoryGen/cometFilter/finetune_language_model/finetuned_model/roc_rl/round_' + str(round - 1)

    args.output_dir += '/round_' + str(round)
    ...
    if device:
        ...
        args.device = torch.device('cuda:' + str(gpu_use))
        device = torch.device('cuda:' + str(gpu_use))
    args.n_gpu = 1
...
if __name__ == "__main__":
    run_finetune_gpt2(gpu_use='0', pho=0.001, round=1)