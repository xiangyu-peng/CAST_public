...
# Loading tf-idf
voc_idf = pickle.load(open("/home/becky/Documents/CommonsenseStoryGen/cometFilter/beam_search/voc_idf.p", "rb"))
...
def diverse_beam(model, length, raw_text, neural=False, num_samples=3, lamb=0.0005, temperature=1):
    global sum_vectors, avg
...
if __name__ == "__main__":
    ...
    for lam in [0.0001, 0.0003, 0.0007, 0.0005]:
        print("Lambda =", lam)
        generated = diverse_beam(model, 30, "Olivia went out with Harry on a date. Harry", num_samples=5, lamb=lam)
        out = decode_beam_search(generated, 25)
        for gen in out.keys():
            print("Olivia went out with Harry on a date.", out[gen])
        print("=================================")
